#!/usr/bin/env python3
"""
EXTRACT COI GENE REGION FROM REFERENCE GENOMES
Matches the region corresponding to your sample sequences
"""
...
def find_coi_region(sample_seq, ref_seq, sample_id=""):
    """
    Find the COI gene region in reference sequence that matches the sample.
    """
    ...
    # Search for patterns in the reference
    for pattern in coi_patterns:
        pos = ref_seq.find(pattern)
        if pos != -1:
            # Extract region similar to sample length
    ...
    for sample_letter in ["A", "B", "C", "D"]:
        ...
        for ref_record in ref_records:
            ...
            if result and score > 20:  # Reasonable match threshold
                ...
                # Fallback: use beginning of sequence
        ...
        # Save extracted references for this sample
        if extracted_records:
            ...
            # Also save with samples for combined file
            ...
            # Add to overall combined records
            all_combined_records.extend(combined_records)

    # Save all extracted sequences together
